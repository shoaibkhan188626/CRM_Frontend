...
import { selectSearchedItems } from '@/redux/crud/selector';

function SearchItemComponent({ config, onRender }) {
  ...
  return (
    <Select
      ...
      showSearch
      allowClear
      placeholder={<SearchOutlined style={{ float: 'right', padding: '8px' }} />}
      defaultActiveFirstOption={false}
      filterOption={false}
      notFoundContent={currentValue}
      value={currentValue}
      onSearch={onSearch}
      style={{ width: '100%' }}
      onSelect={onSelect}
...
export default function SearchItem({ config }) {
  ...
  return state.map((comp) => (
    <SearchItemComponent key={comp} config={config} onRender={onRerender} />
  ));
}
