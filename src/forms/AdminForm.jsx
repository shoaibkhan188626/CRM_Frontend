...
export default function AdminForm({ isUpdateForm = false }) {
  ...
  return (
    <>
      <Form.Item label={translate('first Name')} name="name" rules={[{ required: true }]}>
      ...
      {!isUpdateForm && (
        <Form.Item label={translate('Role')} name="role" rules={[{ required: true }]}>
      ...
      >
        <Select>
          ...
          <Select.Option value="reaedOnly">{translate('read_only')}</Select.Option>
        </Select>
      </Form.Item>
      <Form.Item
        name="file"
